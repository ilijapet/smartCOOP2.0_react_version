...
/**
 * This function checks if the user has a wallet connected and if so, it checks if the user is an admin, cooperant or bidder.
 * @returns The connect button and the disconnect button.
 */
const ConnectButton = () => {
  ...
  const checkWalletIsConnected = async () => {
    ...
    if (ethereum.chainId !== "0x2a") {
      alert(
        "If you would like to use this site please change your network in MetaMask to KOVAN network"
      );
    }
  };
  ...
  const selection = async () => {
    ...
    let user_balance = await coopContract.methods
      .getUserAccountBalance(address)
      .call();

    if (user_balance[0] !== "0") {
      setCooperant(!cooperant);
    } else if (address === "0x273f4FCa831A7e154f8f979e1B06F4491Eb508B6") {
      setAdmin(!admin);
    } else {
      setBidder(!bidder);
    }
